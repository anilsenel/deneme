...
const AI_TIMEOUT = 30000;   // tek bir denemenin üst sınırı (ms)
...
async function aiCall(mode, body, ms = AI_TIMEOUT) {
  const a = DB.settings.ai, base = aiURL();
  const g = timedSignal(ms), t0 = Date.now();
  try {
    const res = mode === 'query'
      ? await fetch(`${base}&subscription-key=${encodeURIComponent(a.key)}`,
        { method: 'POST', body: new Blob([JSON.stringify(body)]), signal: g.signal })
      : await fetch(base, {
        method: 'POST',
    ...
    return { err: e, ms: Date.now() - t0, aborted: e.name === 'AbortError' };
  } finally { g.clear(); }
}

/* Ağ hatasının türünü ayırt eder: zaman aşımı mı, anında ret mi?
   İkisi de JavaScript'e aynı istisnayla gelir, ayrımı süre yapar.        */
function netError(r) {
...
async function aiFetch(body) {
  ...
  for (let i = 0; i < modes.length; i++) {
    const r = await aiCall(modes[i], body, i === 0 ? AI_TIMEOUT : 15000);
    if (r.err) {
      last = netError(r);
      // Zaman aşımı ağ seviyesinde bir engeldir; ikinci yöntem de aynı adrese
      // gideceği için tekrar beklemenin anlamı yok.
      if (r.aborted || r.ms > AI_TIMEOUT - 1500) throw last;
      continue;
    }
    if (r.res.ok) {
      if (a.authMode !== modes[i]) { a.authMode = modes[i]; save(); }
      return r.res.json();
    }
    const t = await r.res.text().catch(() => '');
    last = new Error(`Servis hatası (${r.res.status}). ${t.slice(0, 180)}`);
    if (![401, 403, 404].includes(r.res.status)) throw last;
  }
  throw last;
}
...
async function aiDiagnose() {
  const a = DB.settings.ai;
  let host = '(adres okunamadı)';
  try { host = new URL(a.endpoint).origin; } catch (e) { }
  const lines = [];
  const put = (k, v) => { lines.push(`${k}: ${v}`); paint(); };
  const paint = () => {
  ...
  openModal({
    title: 'Tanılama Raporu', sub: 'Bağlantının nerede kesildiğini adım adım tespit eder', icon: 'i-alert', size: 'modal-lg',
    body: `<div class="ai-busy"><span class="spin"></span> Testler çalışıyor, 1 dakika kadar sürebilir…</div>
      <pre class="ai-out" id="diagOut" style="margin-top:14px;font-family:ui-monospace,Consolas,monospace;font-size:12px"></pre>`,
    foot: `<button class="btn" onclick="closeModal()">Kapat</button>
      <button class="btn btn-primary" onclick="copyText($('diagOut').textContent)"><svg class="ic"><use href="#i-copy"/></svg> Raporu Kopyala</button>`
  });

  put('Tarih', new Date().toLocaleString('tr-TR'));
  put('Sayfa adresi', location.href.slice(0, 120));
  put('Tarayıcı çevrimiçi mi', navigator.onLine ? 'evet' : 'HAYIR — bilgisayarın ağ bağlantısı kapalı');
  put('Servis adresi', host);
  put('Deployment', a.deployment + ' · ' + a.apiVersion);
  put('Anahtar', a.key ? a.key.slice(0, 4) + '…' + a.key.slice(-4) + ` (${a.key.length} karakter)` : 'GİRİLMEMİŞ');
  lines.push('');

  // 1) Sunucuya ağ üzerinden erişilebiliyor mu? (yanıt okunmaz, sadece bağlantı denenir)
  put('1 · Sunucuya erişim', 'deneniyor…');
  const g = timedSignal(15000); const t0 = Date.now();
  let reach = false;
  try {
    await fetch(host + '/', { mode: 'no-cors', cache: 'no-store', signal: g.signal });
    reach = true;
    lines[lines.length - 1] = `1 · Sunucuya erişim: BAŞARILI (${Date.now() - t0} ms)`;
  } catch (e) {
    lines[lines.length - 1] = `1 · Sunucuya erişim: BAŞARISIZ (${Math.round((Date.now() - t0) / 1000)} sn, ${e.name})`;
  } finally { g.clear(); paint(); }

  // 2) Gerçek istek — adres satırında anahtar
  const probe = { messages: [{ role: 'user', content: 'test' }], max_completion_tokens: 5 };
  put('2 · İstek (adres satırı yöntemi)', 'deneniyor…');
  const r1 = await aiCall('query', probe);
  lines[lines.length - 1] = `2 · İstek (adres satırı yöntemi): ` +
    (r1.err ? `BAŞARISIZ — ${r1.aborted ? 'zaman aşımı' : r1.err.name} (${Math.round(r1.ms / 1000)} sn)`
      : `HTTP ${r1.res.status} (${r1.ms} ms)`);
  paint();

  // 3) Gerçek istek — api-key başlığı (preflight gerektirir)
  put('3 · İstek (api-key başlığı)', 'deneniyor…');
  const r2 = await aiCall('header', probe, 15000);
  lines[lines.length - 1] = `3 · İstek (api-key başlığı): ` +
    (r2.err ? `BAŞARISIZ — ${r2.aborted ? 'zaman aşımı' : r2.err.name} (${Math.round(r2.ms / 1000)} sn)`
      : `HTTP ${r2.res.status} (${r2.ms} ms)`);
  paint();

  // Sonuç
  lines.push('', '─── DEĞERLENDİRME ───');
  const ok1 = r1.res && r1.res.ok, ok2 = r2.res && r2.res.ok;
  if (ok1 || ok2) {
    lines.push('Bağlantı çalışıyor. Yapay zekâ özellikleri kullanılabilir.');
  } else if (!navigator.onLine) {
    lines.push('Bilgisayarın ağ bağlantısı kapalı. Önce ağa bağlan.');
  } else if (!reach) {
    lines.push(
    ...
    lines.push('Sunucuya erişiliyor ancak istekler tarayıcı tarafından engelleniyor.',
      'Servis tarafında CORS ayarının açılması gerekebilir (OPTIONS isteğine 200 dönmeli).');
  } else {
    lines.push('Sunucu yanıt verdi ama beklenmeyen bir durum var. Yukarıdaki HTTP kodlarını yöneticine ilet.');
  }
  paint();
  const busy = document.querySelector('#modalBody .ai-busy'); if (busy) busy.remove();
}
